...
class Validaciones  {
    ...
    private static function request($parametro, $is_array = false) {
        if(!$is_array){
            return self::evitar_inyeccion_sql(array_key_exists($parametro, $_REQUEST) ? $_REQUEST[$parametro] : null);
        }else{
            $array = array();
            if(isset($_REQUEST[$parametro])){
                for($i = 0; $i < count($_REQUEST[$parametro]) ;$i++ ) {
                    //var_dump($_REQUEST[$parametro][$i]);
                    $array[] = self::evitar_inyeccion_sql(array_key_exists($parametro, $_REQUEST) ? $_REQUEST[$parametro][$i] : null);
                }
            }
            //var_dump($array);
            return $array;
    ...
    public static function errores_validacion_request(array $validaciones, array &$datos=null) {
        ...
        foreach ($validaciones as $columna => $validacion){
                ...
                //Para contemplar si viene un array de valores
                $is_array = false;
                if(preg_match('/(\w{0,})(\[\])$/', $parametro, $matches)){
                    //var_dump($matches);
                    $parametro = $matches[1];
                    $is_array = true;
                }
                ...
                foreach ($secuencia_validador as $validador){
                        if (preg_match("/:\(/", $validador)) { // Validacion de lista
                                ...
                                if (preg_match("/;/",$validador_partes[1] ))
                                        ...
                                        $opciones=explode(",",$validador_partes[1]);
                                $resultados_validacion["values"][$parametro] = self::request($parametro, $is_array); // La función parametro($parametro) devuelve un valor o null
                                if(!$is_array){
                                    $resultado_validador=self::$validador($resultados_validacion["values"][$parametro], $opciones);
                                }else{
                                    foreach ($resultados_validacion["values"][$parametro] as $value) {
                                        $resultado_validador=self::$validador($value, $opciones);
                                    }
                                }
                        }
...
echo 'Linea: '.__LINE__.'$valores => ';  var_dump($valores);
                                ...
                                if(!$is_array){
                                    ...
                                    foreach ($valores as $key => $value) {
                                        $resultado_validador[]=self::$validador($value, $tabla, $tabla_columnas);
                                    }
                                }
                        }
                        elseif (preg_match("/longitud/", $validador)) { // Validación de longitud
                                ...
                                $resultados_validacion["values"][$parametro] = self::request($parametro, $is_array);
                               
                                if(!$is_array){
                                    $resultado_validador=self::$validador($resultados_validacion["values"][$parametro], $rangos);
                                }else{
                                    foreach ($resultados_validacion["values"][$parametro] as $value) {
                                        $resultado_validador[]=self::$validador($value, $rangos);
                                    }
                                }
                        }
                        else {
                                ...
                                if(!$is_array){
                                    ...
                                    foreach ($resultados_validacion["values"][$parametro] as $value) {
                                        $resultado_validador[]=self::$validador($value);
                                    }
                                }
                        }
echo 'Linea: '.__LINE__.'$resultado_validador => ';  var_dump($resultado_validador);                        
                        if ($resultado_validador) {// La variable $validador contienen el nombre de un método de validación o validador
                            if(!$is_array){    
                                ...
                                foreach ($resultado_validador as $key => $value) {
                                    if( !is_null($value) && $value != false){
                                        $resultados_validacion["errores"][$parametro][$key]= $value;//." --php: $validador:  ";
                                    }
                                }
                            }
...
echo 'Array final: '.var_dump($resultados_validacion);
    ...
    public static function evitar_inyeccion_sql($input)
    {
		...
		return preg_replace (
                    array('/insert/i' , '/select/i' ,'/update/i' ,'/delete/i', '/drop/i' ,'/script/i' ,'/truncate/i','/union/i', '/\;/i'   )
                    ,array("insert_", "select_",'update_','delete_', 'drop_','script_','truncate_','union_', '\\;'  )
                    ,$input
            );
    ...
    */
     ...
     * @param string $cadena
     */
    public static function errores_numero_entero($cadena = null) {
            ...
            if ($cadena != null ) {                    
                    $patron = '/^\-{0,1}[0-9]{0,}$/';
                    if (!preg_match($patron, $cadena)) {
                            $mensaje.= iText('numero_entero', 'dict_error');
                    }
            }
    ...
    public static function errores_numero_entero_positivo($cadena = null) {
            ...
            if ($cadena != null ) {
                    ...
                    if (preg_match($patron, $cadena)) {
                        $mensaje.= 'Contiene caracteres no válidos. Sólo se admiten números enteros positivos sin signo, ni separador de miles.';
    ...
    public static function errores_precio($cadena=null) {
            ...
            if ( ! is_null($cadena) && strlen($cadena)) {
                    ...
                    if( ! preg_match($patron, $cadena))
                        $mensaje = "Error: El número debe escribirse con separador de miles(.) y coma decimal(,) y máximo dos decimales.";
    ...
    /**
     * Valida un precio en formato europeo, con el . como separador de miles y debe ser un precio entero. También da por válido un número sin separador de miles.
     * Ejemplo 10.501 10501
     * 
     * @param string $cadena
     * @return boolean|string
     */
    public static function errores_precio_entero($cadena=null) {

            $mensaje = null; // Optimista

            if ( ! is_null($cadena) && strlen($cadena)) {
                    $patron="/^(\d{1,3}(\.\d{3}){0,}|\d{1,})$/i";
                    if( ! preg_match($patron, $cadena))
                        $mensaje = "Error: El número debe escribirse con separador de miles(.) y sin decimales.";
                        //$mensaje = iText('precio', 'dict_error');
            }
            return $mensaje ;		
    }
    ...
    /**
     * Valida un número en formato europeo, con el . como separador de miles y la , como separador decimal. También da por válido un número sin separador de miles.
     * Ejemplo 10.001,26 10001,26
     * 
     * @param string $cadena
     * @return boolean|string
     */
    public static function errores_numero_con_coma_decimal($cadena=null) {

            $mensaje = null; // Optimista

            if ( ! is_null($cadena) && strlen($cadena)) {
                    $patron="/^((\d{1,3}(\.\d{3}){0,}|\d{1,})(\,\d{0,}){0,1})$/i";
                    if( ! preg_match($patron, $cadena))
                        $mensaje = "Error: El número debe escribirse con separador de miles(.) y coma decimal(,).";
                        //$mensaje = iText('numero_con_coma_decimal', 'dict_error');
            }
            return $mensaje ;		
    }
    ...
    public static function errores_decimal($cadena=null) {
            ...
            if ( ! is_null($cadena) && strlen($cadena)) {
                    $patron="/^[-]{0,1}(\d{0,})(([\.\,]\d{0,}){0,1})$/i";
                    if( ! preg_match($patron, $cadena))
                        $mensaje = 'Error: El número debe escribirse usando "." o"," como separador decimal y sin separador de miles.';
    ...
    public static function errores_unicidad_insertar(array $valores, $tabla, array $columnas) {
            ...
            if ($no_null) {
                    ...
                    if ($filas && count($filas))
                        $mensaje.= "El/Los valor/es aportado/s <b>[ $valores_aportados ]</b> ya existe/n en la base de datos. Escribe otro valor.";
    ...
    // El siguiente array recoge las validaciones por defecto para cada columna de tablas de base de datos cuyos datos pueden entrar a través de $_REQUEST
    ...
    */
    ...
    /**
     *  Valida un teléfono dado
     *  @autor jergo23@gmail.com
    */
    public static function errores_phone($cadena) {
        
        $mensaje = null; // Optimista

        if ( ! is_null($cadena) && strlen($cadena)) {
            $patron="/^[+]{0,1}([\d]{0,2})([\d]{9,})$/i";
            if( ! preg_match($patron, $cadena))
                $mensaje = 'Debe escribir el número sin espacios en blanco';
                //$mensaje = iText('phone', 'dict_error');
        }
